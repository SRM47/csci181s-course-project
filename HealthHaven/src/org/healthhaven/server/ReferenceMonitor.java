...
public class ReferenceMonitor {
	...
	protected static boolean authorizeRequest(SSLSocket clientSocket, String accountType, String callerId,
			JSONObject json, Connection conn) {
		boolean res = false;
		...
		case "VIEW_RECORD": // doctor or patient
			if (accountType.equals("Patient")) {
				...
				res = AccountDAO.isDoctorAuthorizedToViewPatientData(conn, json.optString("doctorID"),
						json.optString("patientID")).getString("result").equals("SUCCESS");
		...
		case "CREATE_RECORD": // doctor
			res = accountType.equals("Doctor") && AccountDAO
					.isDoctorAuthorizedToViewPatientData(conn, json.optString("doctorID"), json.optString("patientID"))
					.getString("result").equals("SUCCESS");
		...
		case "DEACTIVATE_ACCOUNT": // any user for themselves or admin for everyone
			if (accountType.equals("Superadmin")) {
				...
				if ("VALIDATE_ACCOUNT".equals(json.optString("type"))) {
					...
					JSONObject userIdFromEmail = AccountDAO.verifyUserIdfromEmail(conn, callerId,
		...
		if (clientSocket != null) {
			String logMessage = String.format("request:%s caller_id:%s result:%s reason:%s\n",
					json.optString("request"), callerId, res ? "SUCCESS" : "FAILURE", reason);
